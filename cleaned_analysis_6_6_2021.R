...
od_data_recent <- read.csv("./Data/unintent_od_deaths_until_2019.txt", sep = "\t", stringsAsFactors = FALSE)
...
od_data_recent<- od_data_recent %>% filter(Year > 1999 & Year < 2018) %>% group_by(Year) %>%
  summarise(sum_deaths = sum(Deaths, na.rm = TRUE))

# pdf("./Figures/total_od_deaths_all_paper_3_22_21_2000_2017.pdf")
...
#compute the percentage difference between 2000 and 2017
death_2000 <- main_analysis_data_sum$total_deaths[main_analysis_data_sum$year == 2000]
death_2017 <- main_analysis_data_sum$total_deaths[main_analysis_data_sum$year == 2017]

((death_2017 - death_2000)/death_2000)*100

#compute the percentage difference between 1999 and 2019
death_1999 <- od_data_recent$sum_deaths[od_data_recent$Year == 1999]
death_2019 <- od_data_recent$sum_deaths[od_data_recent$Year == 2019]
...
num_states_with_intervention<-data.frame("Start_Date" = unique((main_analysis_data$Intervention_First_Date[!is.na(main_analysis_data$Intervention_First_Date)])))
numStates<-c()
...
# pdf("Figures/num_states_with_intervention_3_12_21.pdf")
...
total_pop <- main_analysis_data %>% group_by(year = year(Time_Period_Start), State) %>% summarise(pop = unique(population)) %>%
  group_by(year) %>% summarise(sum(pop))
main_analysis_data %>% group_by(year(Time_Period_Start)) %>% summarise(sum_deaths = sum(imputed_deaths)*100000) %>% mutate(sum_deaths/total_pop$`sum(pop)`)

main_analysis_data %>% group_by(State) %>% summarise(min_death_rate = min(imputed_deaths/population*100000),
                                                     max_death_rate = max(imputed_deaths/population*100000)) %>%
  mutate(range_death_rate = max_death_rate - min_death_rate) %>% View()
...
#6/6/2021: add estimates for up to 2019
num_attr_data <- main_analysis_data %>% dplyr::select(Time_Period_Start, State, population, imputed_deaths, Intervention_Redefined,
                                                      Time_Period_ID)
...
pop_by_age_2019_group <- pop_by_age_2019_group[pop_by_age_2019_group$NAME != "District of Columbia",]

od_data_2019 <- read.csv("./Data/od_data_up_to_2019.txt", sep = "\t", stringsAsFactors = FALSE)
od_data_2019$Deaths <- as.numeric(od_data_2019$Deaths)
head(od_data_2019, 50)
dim(od_data_2019)
View(od_data_2019)
od_data_2019<-od_data_2019[!is.na(od_data_2019$Year),] #delete the rows that just contains data set description info
tail(od_data_2019)
sum(is.na(od_data_2019$Deaths))

#need to impute some suppressed data
od_data_2019$Date<-mdy(od_data_2019$Month)
length(unique(od_data_2019$State))
#DC is being counted in this, but we do not have data on prosecutions. Remove these rows
od_data_2019<-od_data_2019[od_data_2019$State!="District of Columbia",]

#interpolate the missing values
od_year_2019 <- read.csv("./Data/od_data_up_to_2019_yearly.txt", sep = "\t", stringsAsFactors = FALSE)
od_year_2019 <- od_year_2019[!is.na(od_year_2019$Year),] 
od_year_2019$Deaths <- as.numeric(od_year_2019$Deaths)
head(od_year_2019, 50)
#see how many states have missing yearly entries and use the totals to impute the missing yearly values.
sum_na_2019 <- od_year_2019 %>% group_by(State) %>% summarise(sum(is.na(Deaths)))
table(sum_na_2019)

od_data_2019$interp_vals<-rep(NA, nrow(od_data_2019))

startYear<-2018

for(state in unique(od_data_2019$State)){
  #get the values of the deaths for state
  tempVal<-od_data_2019$Deaths[od_data_2019$State == state]
  #linearly interpolate the deaths that are missing
  tempInterp<-na.approx(tempVal, rule = 2)
  if(sum(!is.na(tempVal)) == 1){
    tempInterp <- tempVal
  }
  for(year in startYear:2019){
    ...
    if(length(indexMissing) != 0){
      ...
      numNotAccounted<-od_year_2019$Deaths[od_year_2019$State == state & od_year_2019$Year == year] - tempDataSum
  ...
  od_data_2019$interp_vals[od_data_2019$State == state]<-tempInterp
}

od_data_2019_summary <- od_data_2019 %>% group_by(State, date = lubridate::floor_date(Date , "6 months" ))  %>%
  summarise("imputed_deaths" = sum(interp_vals)) %>% arrange(date)
intervention_vec <- main_analysis_data %>% arrange(Time_Period_Start, State) %>% filter(Time_Period_Start == "2017-07-01") %>%
  pull(Intervention_Redefined)
sub_data_2019 <- data.frame("Time_Period_Start" = c(rep(as.Date("2018-01-01"), 50), rep(as.Date("2018-07-01"), 50),
                                                    rep(as.Date("2019-01-01"), 50), rep(as.Date("2019-07-01"), 50)),
                            "State" = rep(unique(od_data_2019$State), 4), 
                            "population" = c(rep(pop_by_age_2019_group$pop_2018, 2), rep(pop_by_age_2019_group$pop_2019, 2)),
                            "imputed_deaths" = od_data_2019_summary$imputed_deaths, 
                            "Intervention_Redefined" = rep(intervention_vec, 4),
                            "Time_Period_ID" = c(rep(37:40, each = 50)))
                            
attr_data_2019 <- rbind(num_attr_data, sub_data_2019)
...
attr_data_2019_main_analysis<-attr_data_2019[which(attr_data_2019$Intervention_Redefined>0),]

#compute the probability of overdose had intervention not occurred
prob_od_no_int_main_analysis<-expit(-coef(main_analysis_model)["Intervention_Redefined"]*attr_data_2019_main_analysis$Intervention_Redefined
                                    + logit(attr_data_2019_main_analysis$imputed_deaths/attr_data_2019_main_analysis$population))
...
prob_od_no_int_LB_main_analysis<-expit(-coef_lb[names(coef_lb) == "Intervention_Redefined"]*attr_data_2019_main_analysis$Intervention_Redefined
                                       + logit(attr_data_2019_main_analysis$imputed_deaths/attr_data_2019_main_analysis$population))

prob_od_no_int_UB_main_analysis<-expit(-coef_ub[names(coef_ub) == "Intervention_Redefined"]*attr_data_2019_main_analysis$Intervention_Redefined
                                       + logit(attr_data_2019_main_analysis$imputed_deaths/attr_data_2019_main_analysis$population))


#estimate the number of deaths attributable to the intervention
#first, initialize the vectors to store the numbers
num_attr_od_UB<-num_attr_od_LB<-num_attr_od<-rep(NA, length(unique(attr_data_2019_main_analysis$Time_Period_ID)))
...
for(time in sort(unique(attr_data_2019_main_analysis$Time_Period_ID))){
  #find the indices of rows where the time point = time
  time_point_index<-which(attr_data_2019_main_analysis$Time_Period_ID == time)
  ...
  num_attr_od[index]<-sum(attr_data_2019_main_analysis$imputed_deaths[time_point_index]
                          - prob_od_no_int_main_analysis[time_point_index]*attr_data_2019_main_analysis$population[time_point_index])
  #find the lower and upper bounds of the estimated number of deaths attributable to the intervention
  num_attr_od_LB[index]<-sum(attr_data_2019_main_analysis$imputed_deaths[time_point_index]
                             - prob_od_no_int_LB_main_analysis[time_point_index]*attr_data_2019_main_analysis$population[time_point_index])
  num_attr_od_UB[index]<-sum(attr_data_2019_main_analysis$imputed_deaths[time_point_index]
                             - prob_od_no_int_UB_main_analysis[time_point_index]*attr_data_2019_main_analysis$population[time_point_index])
  index<-index + 1
}

num_attr_od_main_analysis_2019<-data.frame("Time_Period_ID" = sort(unique(attr_data_2019_main_analysis$Time_Period_ID)),
                                      "Time_Start" = sort(unique(attr_data_2019_main_analysis$Time_Period_Start)),
...
sum(num_attr_od_main_analysis_2019$Num_Attr_Deaths)
summary(num_attr_od_main_analysis_2019$Num_Attr_Deaths)
num_attr_od_main_analysis_2019$Time_Start<-as.Date(num_attr_od_main_analysis_2019$Time_Start)

#compute the 95% CI for the total
sum(num_attr_od_main_analysis_2019$Num_Attr_Deaths_LB)
sum(num_attr_od_main_analysis_2019$Num_Attr_Deaths_UB)

#sum up the number of excess deaths per year
yearly_num_Attr_Deaths_main_analysis_2019<-num_attr_od_main_analysis_2019 %>%
...
summary(yearly_num_Attr_Deaths_main_analysis_2019$deaths)
summary(yearly_num_Attr_Deaths_main_analysis_2019$death_lb)
summary(yearly_num_Attr_Deaths_main_analysis_2019$death_ub)

#just for up to 2017
attr_deaths_anlys_od_all <- attr_data_2019_main_analysis[year(attr_data_2019_main_analysis$Time_Period_Start) < 2018,]
num_attr_od_main_analysis<-data.frame("Time_Period_ID" = sort(unique(attr_deaths_anlys_od_all$Time_Period_ID)),
                                      "Time_Start" = sort(unique(attr_deaths_anlys_od_all$Time_Period_Start)),
                                      "Num_Attr_Deaths" = num_attr_od[1:36],
                                      "Num_Attr_Deaths_LB" = num_attr_od_LB[1:36],
                                      "Num_Attr_Deaths_UB" = num_attr_od_UB[1:36])

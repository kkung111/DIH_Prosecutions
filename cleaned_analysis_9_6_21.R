...
od_data_recent <- read.csv("./Data/unintentional_od_yearly_1999_2019_17_up.txt", sep = "\t", stringsAsFactors = FALSE)
...
od_data_recent<- od_data_recent %>% filter(Year > 1999 & Year < 2020) %>% group_by(Year) %>%
...
#get a vector of the number of states with an intervention at every 6 month period
num_states_w_intervention_6_months <- num_states_with_intervention %>%
  filter(numStates == "numStates") %>%
  mutate(Date = lubridate::floor_date(Start_Date, "6 months" )) %>%
  group_by(Date) %>%
  summarise(total_num_states = max(value))

#merge the total number of states with DIH prosecutions into dataset
main_analysis_data <- merge(main_analysis_data, num_states_w_intervention_6_months, 
                            by.x = "Time_Period_Start", by.y = "Date", all.x = TRUE)
#impute the missing data, which is just the previous count of states with prosecution
main_analysis_data <- main_analysis_data %>%
  group_by(State) %>%
  mutate(total_num_states = ifelse(is.na(total_num_states), lag(total_num_states), total_num_states))

#date when 49 states have prosecution
min_49_states_date <- min(main_analysis_data$Time_Period_Start[main_analysis_data$total_num_states == 49], na.rm = TRUE)
main_analysis_data$total_num_states[main_analysis_data$Time_Period_Start > min_49_states_date] <- 49
...
main_analysis_data %>% 
  group_by(State) %>% 
  summarise(min_death_rate = min(imputed_deaths/population*100000),
...
rownames(main_analysis_covariate_table)<-c("Naloxone_Pharmacy_Yes", "Naloxone_Pharmacy_No",
                                           ...
                                           "Intervention", "Total Number with DIH Prosecutions")
...
#merge the total number of states with DIH prosecutions into dataset
od_all_data <- merge(od_all_data, num_states_w_intervention_6_months, 
                     by.x = "Time_Period_Start", by.y = "Date", all.x = TRUE)
#impute the missing data, which is just the previous count of states with prosecution
...
#date when 49 states have prosecution
min_49_states_date <- min(od_all_data$Time_Period_Start[od_all_data$total_num_states == 49], na.rm = TRUE)
od_all_data$total_num_states[od_all_data$Time_Period_Start > min_49_states_date] <- 49
...
covariates<-c("Naloxone_Pharmacy_Yes_Redefined", "Naloxone_Pharmacy_No_Redefined",
              ...
              "Recreational_Marijuana_Redefined",
              "GSL_Redefined", "PDMP_Redefined",
              "Medicaid_Expansion_Redefined", "Intervention_Redefined", "num_states_w_intervention")
...
rownames(od_all_covariate_table)<-c("Naloxone_Pharmacy_Yes", "Naloxone_Pharmacy_No",
                                    ...
                                    "Recreational_Marijuana",
                                    "GSL", "PDMP", "Medicaid_Expansion",
                                    "Intervention", "Total Number with DIH Prosecution")
...
rownames(sensitivity_anlys_conf_by_indication_covariate_table)<-c("Naloxone_Pharmacy_Yes", "Naloxone_Pharmacy_No",
                                                                  ...
                                                                  "Total Number of States with DIH Prosecution")
...
covariates<-c("Naloxone_Pharmacy_Yes_Redefined", "Naloxone_Pharmacy_No_Redefined",
              ...
              "GSL_Redefined", "PDMP_Redefined",
              "Medicaid_Expansion_Redefined", "Intervention_Redefined",
              "Total Number of States w Intervention")
...
covariates<-c("Naloxone_Pharmacy_Yes_Redefined", "Naloxone_Pharmacy_No_Redefined",
              ...
              "Recreational_Marijuana_Redefined",
              "GSL_Redefined", "PDMP_Redefined",
              "Medicaid_Expansion_Redefined", "int_2_yr_effect", "num_states_w_intervention_2_yr_effect")
...
rownames(sensitivity_anlys_redefine_int_covariate_table)<-c("Naloxone_Pharmacy_Yes", "Naloxone_Pharmacy_No",
                                                            ...
                                                            "Recreational_Marijuana",
                                                            "GSL", "PDMP", "Medicaid_Expansion",
                                                            "int_2_yr_effect", "Number of States w Intervention")
...
covariates<-c("Naloxone_Pharmacy_Yes_Redefined", "Naloxone_Pharmacy_No_Redefined",
              ...
              "Recreational_Marijuana_Redefined",
              "GSL_Redefined", "PDMP_Redefined",
              "Medicaid_Expansion_Redefined", "int_2_yr_effect_lag", "num_states_w_intervention_2yr_lag")
...
rownames(sens_analysis_2yr_int_lag_covariate_table)<-c("Naloxone_Pharmacy_Yes", "Naloxone_Pharmacy_No",
                                                       ...
                                                       "Recreational_Marijuana",
                                                       "GSL", "PDMP", "Medicaid_Expansion",
                                                       "Intervention", "Number of States w Intervention")

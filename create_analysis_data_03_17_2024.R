...
pop_by_age_2010_group <- pop_by_age_2010 %>% 
  ...
  #filter to age below 999 because that is for the total
  filter(NAME != "United States", 
         SEX == 0, 
         AGE >= 18, 
         AGE < 999) %>%
  group_by(NAME) %>%
  summarise(pop_2000 = sum(ESTIMATESBASE2000[SEX == 0]),
...
od_data <- read.csv("./Data/od_unintentional_18_and_up_11_28_21.txt", sep = "\t", stringsAsFactors = FALSE)
...
od_year <- read.csv("./Data/od_unintentional_yearly_18_and_up_11_28_21.txt", sep = "\t", stringsAsFactors = FALSE)
...
prosec_data<-prosec_data %>% 
  ...
  #code the NA as 9999 for now, otherwise they will be excluded
  ...
  filter(!is.na(Date), State.Filed != "No Info", State.Filed != "No info", State.Filed != "No Info ",
         deceased_age >= 18) %>%
  #change the ages from 9999 to NA
...
prosec_grouped_data<-prosec_data %>% 
...
#LA dispensed first medical marijuana on June 29, 2015: https://web.archive.org/web/20150831120311/http://www.nola.com/politics/index.ssf/2015/06/bobby_jindal_marijuana_laws.html
medMar_start_dates[names(medMar_start_dates) == "Louisiana"] <- as.Date("2015-06-29")
...
#MN -#Checked with data on PDAPS, and I believe that there is a typo in the Minnesota date. The actual first effective date is May 30, 2014
medMar_start_dates[names(medMar_start_dates) == "Minnesota"] <- as.Date("2014-05-30")
...
#colorado- amendment 64 passed in nov 6. 2012 https://www.denverpost.com/2012/11/06/coloradans-say-yes-to-recreational-use-of-marijuana/
#washington - https://www.denverpost.com/2012/11/06/coloradans-say-yes-to-recreational-use-of-marijuana/ - inside the text
#CA: nov 8, 2016: https://ballotpedia.org/California_Proposition_64,_Marijuana_Legalization_(2016)
recMar_start_dates[names(recMar_start_dates) == "Colorado"]<-as.Date("2012-11-06")
recMar_start_dates[names(recMar_start_dates) == "Washington"]<-as.Date("2012-11-06")
...
write.csv(full_data_set, "./Data/full_data_set_03_18_2024_unintentional.csv", row.names = FALSE)

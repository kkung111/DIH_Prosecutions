...
compute_sd_and_CI <- function(cov_data, observed_y, coef_values, z_value = 1.96,
  ...
  var_cov <- solve(crossprod(cov_data))%*%(middle_term)%*%solve(crossprod(cov_data))
  #we obtain the standard deviations by taking the square root of the diagonal of the variance-covariance matrix.
  sd_of_coefficients <- sqrt(diag(var_cov))
...
list_of_states <- unique(main_analysis_data$State)
sensitivity_anlys_exclude_diff_states_tab_results <- data.frame(state = c("", list_of_states), 
                                                                coefficient_intervention = rep(NA, 51),
                                                                std_error = rep(NA, 51),
                                                                ci_lb = rep(NA, 51),
                                                                ci_ub = rep(NA, 51))
for(state in sensitivity_anlys_exclude_diff_states_tab_results$state){
  #include only the states that are not excluded
  ...
  #run analysis with the state excluded
  ...
  sensitivity_anlys_exclude_diff_states_tab_results$coefficient_intervention[sensitivity_anlys_exclude_diff_states_tab_results$state ==
                                                                               state] <-
    summary_sensitivity_analys_exclude_diff_states$p.coeff["Intervention_Redefined"]
  
  #sandwich estimator
  #compute the full dataset including basis functions
  sensitivity_anlys_exclude_diff_states_w_basis_functions <- data.frame(predict(sensitivity_anlys_exclude_diff_states_model, 
                                                                                type = "lpmatrix"))

  #estimate the 95% CI and SD
  sensitivity_anlys_exclude_diff_states_coefficient_values <- coef(sensitivity_anlys_exclude_diff_states_model)
  #type = "response" to get the estimated probabilities
  ssensitivity_anlys_exclude_diff_states_pred_prob <- predict(sensitivity_anlys_exclude_diff_states_model, 
                                                          newdata = sensitivity_anlys_exclude_diff_states_data, type = "response")
  sensitivity_anlys_exclude_diff_states_sd_and_ci <- compute_sd_and_CI(sensitivity_anlys_exclude_diff_states_w_basis_functions, 
                                             sensitivity_anlys_exclude_diff_states_data$population,
                                             sensitivity_anlys_exclude_diff_states_data$imputed_deaths,
                                             ssensitivity_anlys_exclude_diff_states_pred_prob, 
                                             sensitivity_anlys_exclude_diff_states_coefficient_values)

  #obtain standard error of intervention
  sensitivity_anlys_exclude_diff_states_tab_results$std_error[sensitivity_anlys_exclude_diff_states_tab_results$state == state] <- 
    sensitivity_anlys_exclude_diff_states_sd_and_ci$sd_coef[rownames(sensitivity_anlys_exclude_diff_states_sd_and_ci) == 
                                                              "Intervention_Redefined"]
  #obtain LB of 95% CI
  sensitivity_anlys_exclude_diff_states_tab_results$ci_lb[sensitivity_anlys_exclude_diff_states_tab_results$state == state] <- 
    sensitivity_anlys_exclude_diff_states_sd_and_ci$lb_coef[rownames(sensitivity_anlys_exclude_diff_states_sd_and_ci) == 
                                                              "Intervention_Redefined"]
  
    #obtain UBB of 95% CI
  sensitivity_anlys_exclude_diff_states_tab_results$ci_ub[sensitivity_anlys_exclude_diff_states_tab_results$state == state] <- 
    sensitivity_anlys_exclude_diff_states_sd_and_ci$ub_coef[rownames(sensitivity_anlys_exclude_diff_states_sd_and_ci) == 
                                                              "Intervention_Redefined"]
}

sensitivity_anlys_exclude_diff_states_tab_results$state[sensitivity_anlys_exclude_diff_states_tab_results$state == ""] <- 
  "No States Excluded"
sensitivity_anlys_exclude_diff_states_tab_results

plot_state_effects <- sensitivity_anlys_exclude_diff_states_tab_results %>%
  dplyr::select(-std_error)

colnames(plot_state_effects) <- c("term", "estimate", "conf.low", "conf.high")

# write.csv(plot_state_effects, "./Data/coef_est_excluding_diff_states_1_18_22.csv", 
#           row.names = FALSE)
```

```{r}
plot_state_effects <- read.csv("./Data/coef_est_excluding_diff_states_1_18_22.csv")
# pdf("./Figures/coef_plot_excluding_diff_states_1_18_22.pdf")
dwplot(plot_state_effects, colour = "black") +  
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_vline(aes(xintercept = 0), linetype = "dashed") +
  labs(y = "States Excluded", x = "Coefficients and 95% Confidence Intervals", 
       title = "Coefficient of Intervention of Interest, Excluding One State at a Time") + 
  scale_color_grey()
# dev.off()
```
...
```
# Sensitvity Analysis 6b: Study of Periods with Indicators At Each Period After Treatment
```{r}
sensitivity_anlys_event_study_data$sum_12_years <- (sensitivity_anlys_event_study_data$pos_25_pd + 
                                                      sensitivity_anlys_event_study_data$pos_26_pd + 
                                                      sensitivity_anlys_event_study_data$pos_27_pd + 
                                                      sensitivity_anlys_event_study_data$pos_28_pd + 
                                                      sensitivity_anlys_event_study_data$pos_29_pd + 
                                                      sensitivity_anlys_event_study_data$pos_30_pd + 
                                                      sensitivity_anlys_event_study_data$pos_31_pd + 
                                                      sensitivity_anlys_event_study_data$pos_32_pd + 
                                                      sensitivity_anlys_event_study_data$pos_33_pd + 
                                                      sensitivity_anlys_event_study_data$pos_34_pd + 
                                                      sensitivity_anlys_event_study_data$pos_35_pd + 
                                                      sensitivity_anlys_event_study_data$pos_36_pd + 
                                                      sensitivity_anlys_event_study_data$pos_37_pd + 
                                                      sensitivity_anlys_event_study_data$pos_38_pd + 
                                                      sensitivity_anlys_event_study_data$pos_39_pd)
# formula_post_tx <- formula(paste("cbind(round(imputed_deaths), round(num_alive))~ State +
#                                            s(Time_Period_ID, bs = 'cr', by = as.factor(Region))  +
#                                            Naloxone_Pharmacy_Yes_Redefined +
#                                            Naloxone_Pharmacy_No_Redefined +
#                                            Medical_Marijuana_Redefined +
...
#type = "response" to get the estimated probabilities
pred_prob_sensitivity_anlys_post_tx <- predict(sensitivity_anlys_post_tx_model, 
                                                   newdata = sensitivity_anlys_event_study_data, type = "response")
# sensitivity_anlys_post_tx_sd_and_ci <- compute_sd_and_CI(full_df_w_basis_functions_sensitivity_anlys_post_tx, 
#                                                               sensitivity_anlys_event_study_data$population,
#                                                               sensitivity_anlys_event_study_data$imputed_deaths,
#                                                               pred_prob_sensitivity_anlys_post_tx, 
...
sensitivity_anlys_post_tx_sd_and_ci <- compute_sd_and_CI(as.matrix(full_df_w_basis_functions_sensitivity_anlys_post_tx), 
                                                         sensitivity_anlys_event_study_data$prop_dead,
                                                         coefficient_values_sensitivity_anlys_post_tx)
round(sensitivity_anlys_post_tx_sd_and_ci, 3)
```
...
# Sensitvity Analysis 6c: Semi-Dynamic Regression
```{r}
dw_plot <- data.frame(matrix(NA, nrow = 38, ncol = 4))
for(i in 0:37){
  time_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                     s(Time_Period_ID, bs = "cr", by = as.factor(Region), k = 3) +
                     Naloxone_Pharmacy_Yes_Redefined +
                     Naloxone_Pharmacy_No_Redefined +
                     Medical_Marijuana_Redefined +
                     Recreational_Marijuana_Redefined +
                     ...
                     num_states_w_intervention,
                   data = sensitivity_anlys_event_study_data[
                     sensitivity_anlys_event_study_data[,paste("pos", i, "pd", sep = "_")] == 1 |
                       sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], 
                   family = "binomial")
  
  full_df_time_coef <- data.frame(predict(time_coef, type = "lpmatrix"))
  
  #estimate the 95% CI and SD
  coefficient_values_time_coef <- coef(time_coef)
  #type = "response" to get the estimated probabilities
  pred_prob_time_coef <- predict(time_coef, 
                                 newdata = sensitivity_anlys_event_study_data[
                                   sensitivity_anlys_event_study_data[,paste("pos", i, "pd", sep = "_")] == 1 |
                                     sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], type = "response")
  print(i)
  sensitivity_anlys_time_coef <- compute_sd_and_CI(full_df_time_coef, 
                                                   sensitivity_anlys_event_study_data$population[
                                                     sensitivity_anlys_event_study_data[,paste("pos", i, "pd", sep = "_")] == 1 |
                                                       sensitivity_anlys_event_study_data$Intervention_Redefined == 0],
                                                   sensitivity_anlys_event_study_data$imputed_deaths[
                                                     sensitivity_anlys_event_study_data[,paste("pos", i, "pd", sep = "_")] == 1 |
                                                       sensitivity_anlys_event_study_data$Intervention_Redefined == 0],
                                                   pred_prob_time_coef, 
                                                   coefficient_values_time_coef)
  
  dw_plot[i+1,] <- c(i, sensitivity_anlys_time_coef["Intervention_Redefined", 1:3])
  print(dw_plot[i+1,])
}

colnames(dw_plot) <- c("term", "conf.low", "estimate", "conf.high")
dwplot(dw_plot)

df_to_plot <- data.frame(time_point = seq(0, 34), coef = rep(NA, 35), lb = rep(NA, 35), ub = rep(NA, 35))
states_0 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_0_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0])
time_0_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
                             ...
                             Intervention_Redefined + 
                             num_states_w_intervention,
                           data = sensitivity_anlys_event_study_data[sensitivity_anlys_event_study_data$pos_0_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], 
                   family = "binomial")


df_to_plot$coef[df_to_plot$time_point == 0] <- coef(time_0_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 0] <- coef(time_0_coef)["Intervention_Redefined"] - 
  1.96*summary(time_0_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 0] <- coef(time_0_coef)["Intervention_Redefined"] + 
  1.96*summary(time_0_coef)$se["Intervention_Redefined"]

states_1 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_1_pd == 1 | 
                                                              sensitivity_anlys_event_study_data$State == "Hawaii"])
time_1_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
                             ...
                             Intervention_Redefined + 
                             num_states_w_intervention,
                           data = sensitivity_anlys_event_study_data[sensitivity_anlys_event_study_data$pos_1_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], 
                   family = "binomial")

df_to_plot$coef[df_to_plot$time_point == 1] <- coef(time_1_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 1] <- coef(time_1_coef)["Intervention_Redefined"] - 
  1.96*summary(time_1_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 1] <- coef(time_1_coef)["Intervention_Redefined"] + 
  1.96*summary(time_1_coef)$se["Intervention_Redefined"]

states_2 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_2_pd == 1 | 
                                                              sensitivity_anlys_event_study_data$State == "Hawaii"])
time_2_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
                             ...
                             Intervention_Redefined + 
                             num_states_w_intervention,
                           data = sensitivity_anlys_event_study_data[sensitivity_anlys_event_study_data$pos_2_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0,,], 
                   family = "binomial")

df_to_plot$coef[df_to_plot$time_point == 2] <- coef(time_2_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 2] <- coef(time_2_coef)["Intervention_Redefined"] - 
  1.96*summary(time_2_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 2] <- coef(time_2_coef)["Intervention_Redefined"] + 
  1.96*summary(time_2_coef)$se["Intervention_Redefined"]

states_3 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_3_pd == 1 | 
                                                              sensitivity_anlys_event_study_data$State == "Hawaii"])
time_3_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
                             ...
                             Intervention_Redefined + 
                             num_states_w_intervention,
                           data = sensitivity_anlys_event_study_data[sensitivity_anlys_event_study_data$pos_3_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], 
                   family = "binomial")

df_to_plot$coef[df_to_plot$time_point == 3] <- coef(time_3_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 3] <- coef(time_3_coef)["Intervention_Redefined"] - 
  1.96*summary(time_3_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 3] <- coef(time_3_coef)["Intervention_Redefined"] + 
  1.96*summary(time_3_coef)$se["Intervention_Redefined"]

states_5 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_5_pd == 1 | 
                                                              sensitivity_anlys_event_study_data$State == "Hawaii"])
time_5_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
                             ...
                             Intervention_Redefined + 
                             num_states_w_intervention,
                           data = sensitivity_anlys_event_study_data[sensitivity_anlys_event_study_data$pos_5_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], 
                   family = "binomial")

df_to_plot$coef[df_to_plot$time_point == 5] <- coef(time_5_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 5] <- coef(time_5_coef)["Intervention_Redefined"] - 
  1.96*summary(time_5_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 5] <- coef(time_5_coef)["Intervention_Redefined"] + 
  1.96*summary(time_5_coef)$se["Intervention_Redefined"]

states_6 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_6_pd == 1 | 
                                                              sensitivity_anlys_event_study_data$State == "Hawaii"])
time_6_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
                             ...
                             Intervention_Redefined + 
                             num_states_w_intervention,
                           data = sensitivity_anlys_event_study_data[sensitivity_anlys_event_study_data$pos_6_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], 
                   family = "binomial")

df_to_plot$coef[df_to_plot$time_point == 6] <- coef(time_6_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 6] <- coef(time_6_coef)["Intervention_Redefined"] - 
  1.96*summary(time_6_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 6] <- coef(time_6_coef)["Intervention_Redefined"] + 
  1.96*summary(time_6_coef)$se["Intervention_Redefined"]

states_7 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_7_pd == 1 | 
                                                              sensitivity_anlys_event_study_data$State == "Hawaii"])
time_7_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
...
df_to_plot$coef[df_to_plot$time_point == 7] <- coef(time_7_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 7] <- coef(time_7_coef)["Intervention_Redefined"] - 
  1.96*summary(time_7_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 7] <- coef(time_7_coef)["Intervention_Redefined"] + 
  1.96*summary(time_7_coef)$se["Intervention_Redefined"]

states_12 <- unique(sensitivity_anlys_event_study_data$State[sensitivity_anlys_event_study_data$pos_12_pd == 1 | 
                                                              sensitivity_anlys_event_study_data$State == "Hawaii"])
time_12_coef <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                             s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                             Naloxone_Pharmacy_Yes_Redefined +
                             Naloxone_Pharmacy_No_Redefined +
                             Medical_Marijuana_Redefined +
                             ...
                             Intervention_Redefined + 
                             num_states_w_intervention,
                           data = sensitivity_anlys_event_study_data[sensitivity_anlys_event_study_data$pos_12_pd == 1 |
                                                              sensitivity_anlys_event_study_data$Intervention_Redefined == 0,], family = "binomial")

df_to_plot$coef[df_to_plot$time_point == 12] <- coef(time_12_coef)["Intervention_Redefined"]
df_to_plot$lb[df_to_plot$time_point == 12] <- coef(time_12_coef)["Intervention_Redefined"] - 
  1.96*summary(time_12_coef)$se["Intervention_Redefined"]
df_to_plot$ub[df_to_plot$time_point == 12] <- coef(time_12_coef)["Intervention_Redefined"] + 
  1.96*summary(time_12_coef)$se["Intervention_Redefined"]
```


```{r}
ggplot(df_to_plot, aes(y = coef, x = time_point)) + geom_point() + 
...
dwplot(plot_post_tx, colour = "black",
       vars_order =  c(sapply((max(merged_main_time_data_int$intervention_time_id, na.rm = TRUE):0), 
                                   function(x){paste("pos_", x, "_pd", sep = "")}))) +  
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), 
        axis.text.x = element_text(angle = 45)) +
...
#cr is used for cubic regression spline -- we are smoothing time effects by region
#run the analysis for all the states
sensitivity_anlys_poiss_model<-gam(round(imputed_deaths)~ State +
                                     s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                                     Naloxone_Pharmacy_Yes_Redefined +
                                     Naloxone_Pharmacy_No_Redefined +
                                     Medical_Marijuana_Redefined +
...
```{r}
#compute the full dataset including basis functions
sensitivity_anlys_poiss_w_basis <- data.frame(predict(sensitivity_anlys_poiss_model, type = "lpmatrix"))

#estimate the 95% CI and SD
sensitivity_anlys_poiss_coefficient_values <- coef(sensitivity_anlys_poiss_model)
#type = "response" to get the estimated probabilities
sensitivity_anlys_poiss_pred_prob <- predict(sensitivity_anlys_poiss_model, 
                                                   newdata = main_analysis_data, type = "response")
sensitivity_anlys_poiss_sd_and_ci <- compute_sd_and_CI(sensitivity_anlys_poiss_w_basis, 
                                                              main_analysis_data$population,
                                                              main_analysis_data$imputed_deaths,
                                                              sensitivity_anlys_poiss_pred_prob, 
                                                              sensitivity_anlys_poiss_coefficient_values)
round(sensitivity_anlys_poiss_sd_and_ci, 3)
```

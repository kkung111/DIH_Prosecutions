...
# pdf("./Figures/total_od_18_up_deaths_paper_6_28_22_2000_2019.pdf")
ggplot(data = main_analysis_data_sum, mapping = aes(x = year, y = total_deaths)) +
  geom_line() + 
  geom_point() +
  labs(x = "Year", y = "Yearly Number of Unintentional Drug Overdose Deaths in the 50 U.S. States") +
  theme(panel.background = element_rect("white"), 
...
death_2000/main_analysis_data_sum$sum_pop[main_analysis_data_sum$year == 2000]*100000
death_2019/main_analysis_data_sum$sum_pop[main_analysis_data_sum$year == 2010]*100000
...
#plot number of DIH prosecutions in data
prosecution_yrly_grouped <- prosecution_data_unique %>%
  filter(Deceased.s.Age >= 18,
         year(Date) <= 2019) %>%
  group_by(year = year(Date)) %>%
  summarise(num_dih = n())

# pdf("Figures/yearly_num_of_dih_prosecution_6_6_22.pdf", width = 5, height = 5)
ggplot(prosecution_yrly_grouped, aes(x = year, y = num_dih)) + 
  geom_line() + 
  labs(y = "Yearly Number of DIH Prosecutions Reported in the Media",
       x = "Year") + 
  theme(panel.border = element_blank(), 
        ...
        axis.line = element_line(colour = "black"),
        panel.background = element_rect("white")) + 
  scale_x_continuous(limits = c(1974, 2019), breaks = seq(1974, 2019, 5))
# dev.off()
...
# stargazer(main_analysis_model, type = "html")
...
#First, need to create a lagged variable for the past OD deaths and proportion of deaths
main_analysis_data_with_lagged_deaths <- main_analysis_data %>%
  arrange(State, Time_Period_ID) %>%
  group_by(State) %>%
  mutate(lag_deaths = lag(imputed_deaths),
         lag_prop_death = lag(imputed_deaths/population),
         prop_death = imputed_deaths/population)

#Then fit model to see if OD deaths for state s at time t depends on the past 

# Logistic Model
main_analysis_model_past_OD_logistic <- gam(cbind(round(imputed_deaths), round(num_alive))~ State +
                                              s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                                              Naloxone_Pharmacy_Yes_Redefined +
                                              Naloxone_Pharmacy_No_Redefined +
                                              Medical_Marijuana_Redefined +
                                              ...
                                              num_states_w_intervention + 
                                              lag_prop_death,
                                            data = main_analysis_data_with_lagged_deaths, family = "binomial")
# stargazer(main_analysis_model_past_OD_logistic, type = "html")
summary(main_analysis_model_past_OD_logistic)

# Linear Link Function
main_analysis_model_past_OD_lin_link <- gam(log(prop_death)~ State +
                                              s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                                              Naloxone_Pharmacy_Yes_Redefined +
                                              Naloxone_Pharmacy_No_Redefined +
                                              Medical_Marijuana_Redefined +
                                              ...
                                              num_states_w_intervention + 
                                              lag_prop_death,
                                            data = main_analysis_data_with_lagged_deaths)
# stargazer(main_analysis_model_past_OD_lin_link, type = "html")
summary(main_analysis_model_past_OD_lin_link)
```
...
#print_full_cov indicates whether to return the full variance-covariance matrix

compute_sd_and_CI <- function(cov_data, observed_y, coef_values, l, z_value = 1.96,
  ...
  bias_correcting_term <- (nrow(cov_data)/(nrow(cov_data) - l))
  
  #compute the variance covarianc ematrix
  var_cov <- inverse_C_term%*%as.matrix(middle_term)%*%inverse_C_term*bias_correcting_term
  ...
  #if print_full_cov == TRUE, return full variance covariance matrix
  if(print_full_cov){
...
main_analysis_sd_and_ci <- compute_sd_and_CI(full_df_w_basis_functions, 
                                             (main_analysis_data$prop_dead),
                                             coefficient_values,
                                             l = ncol(full_df_w_basis_functions))
...
# pdf("./Figures/GAM_fitted_vs_actual_by_Region_5_10_22_with_int_date_full_data.pdf", height = 6, width = 6)
...
sum(yearly_num_Attr_Deaths_main_analysis$deaths)/sum(main_analysis_data$imputed_deaths)
sum(yearly_num_Attr_Deaths_main_analysis$death_lb)/sum(main_analysis_data$imputed_deaths)
sum(yearly_num_Attr_Deaths_main_analysis$death_ub)/sum(main_analysis_data$imputed_deaths)

```

## Secondary Analysis: Assuming Effect Lasts for Two Years
### Analysis
```{r, results = "asis"}
...
sensitivity_anlys_redefine_int_sd_and_ci <- compute_sd_and_CI(sensitivity_anlys_redefine_int_w_basis_functions, 
                                                              (sensitivity_anlys_redefine_int_data$prop_dead),
                                                              sensitivity_anlys_redefine_int_coefficient_values,
                                                              l = ncol(sensitivity_anlys_redefine_int_w_basis_functions))
...
sum(yearly_num_Attr_Deaths_redefine_int$deaths)/sum(main_analysis_data$imputed_deaths)
sum(yearly_num_Attr_Deaths_redefine_int$death_lb)/sum(main_analysis_data$imputed_deaths)
sum(yearly_num_Attr_Deaths_redefine_int$death_ub)/sum(main_analysis_data$imputed_deaths)
...
ggplot() + 
  geom_line(yearly_num_Attr_Deaths_redefine_int, mapping = aes(x = year, y = deaths, 
                                                               linetype = "Estimated Effect",
                                                               color = "Two-Year Effect")) + 
  geom_point(yearly_num_Attr_Deaths_redefine_int, mapping = aes(y = deaths, x = year, 
                                                                shape = "Two-Year Effect",
                                                                color = "Two-Year Effect")) + 
  geom_line(yearly_num_Attr_Deaths_redefine_int, mapping = aes(y = death_lb, x = year, 
                                                               linetype = "95% CI for Model with Two-Year Effect",
                                                               color = "Two-Year Effect")) + 
  geom_line(yearly_num_Attr_Deaths_redefine_int, mapping = aes(y = death_ub, x = year, 
                                                               linetype = "95% CI for Model with Two-Year Effect",
                                                               color = "Two-Year Effect")) + 
  geom_line(yearly_num_Attr_Deaths_main_analysis, mapping = aes(x = year, y = deaths, 
                                                                linetype = "Estimated Effect",
                                                                color = "Lasting Effect")) + 
  geom_point(yearly_num_Attr_Deaths_main_analysis, 
             mapping = aes(y = deaths, x = year, 
                           shape = "Lasting Effect",
                           color = "Lasting Effect")) + 
  geom_line(yearly_num_Attr_Deaths_main_analysis, mapping = aes(y = death_lb, x = year, 
                                                                color = "Lasting Effect",
                                                                linetype = "95% CI for Model with Lasting Effect")) + 
  geom_line(yearly_num_Attr_Deaths_main_analysis, mapping = aes(y = death_ub, x = year, 
                                                                color = "Lasting Effect",
                                                                linetype = "95% CI for Model with Lasting Effect")) + 
  theme(panel.background = element_rect("white"), 
        panel.border = element_blank(), 

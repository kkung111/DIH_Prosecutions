...
compute_sd_and_CI_linear_link <- function(cov_data, observed_y, coef_values, z_value = 1.96, k,
  ...
  crossproduct_cov_data_inverse <- solve(crossprod(cov_data))
  var_cov <- crossproduct_cov_data_inverse%*%(middle_term)%*%crossproduct_cov_data_inverse*(nrow(cov_data)/(nrow(cov_data) - k))
...
compute_sd_and_CI_logistic <- function(cov_data, observed_y, coef_values, z_value = 1.96,
                              k,
                              print_full_cov = FALSE){
  C_term <- middle_term <- matrix(0, nrow = ncol(cov_data), ncol = ncol(cov_data))
  for(i in 1:nrow(cov_data)){
    #p_{s,t} = expit(z_{s,t}^T Theta)
    pst <- expit(t(as.matrix(cov_data[i,]))%*%coef_values)
    
    #sum_{s,t} z_{s,t} z_{s,t}^T (y_{s,t} - p_{s,t})
    middle_term <- middle_term + tcrossprod(as.matrix(cov_data[i,]))*
      as.numeric((observed_y[i] - pst)^2)
    
    #sum_{s,t} z_{s,t} z_{s,t}^T p_{s,t}*(1-p_{s,})
    C_term <- C_term + tcrossprod(as.matrix(cov_data[i,]))*as.numeric(pst*(1 - pst))
  }

  inverse_C_term <- solve(C_term)
  
  var_cov <- inverse_C_term%*%as.matrix(middle_term)%*%inverse_C_term*(nrow(cov_data)/(nrow(cov_data) - k))
...
main_analysis_model_log_not_gam<-lm(log(prop_dead)~ State +
                                             # s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                                               Time_Period_ID:as.factor(Region) + 
...
summary(main_analysis_model_log_not_gam)

#examine fitted values
summary(fitted(main_analysis_model_log_not_gam))
hist(fitted(main_analysis_model_log_not_gam))

#smoothed effects
par(mfrow = c(2,2))
plot(main_analysis_model_log_not_gam)
par(mfrow = c(1,1))
```
...
full_df_w_basis_functions_log_not_gam <- model.matrix(main_analysis_model_log_not_gam)

#estimate the 95% CI and SD
coefficient_values_log_not_gam <- coef(main_analysis_model_log_not_gam)
#type = "response" to get the estimated probabilities
main_analysis_sd_and_ci_log_not_gam <- compute_sd_and_CI_linear_link(full_df_w_basis_functions_log_not_gam,
                                                               log(main_analysis_data$prop_dead),
                                                               coefficient_values_log_not_gam,
                                                               k = ncol(full_df_w_basis_functions_log_not_gam))
# format(round(main_analysis_sd_and_ci_log_, 3), nsmall = 3)

colnames(main_analysis_sd_and_ci_log_not_gam) <- c("conf.low", "estimate", "conf.high", "sd")
main_analysis_sd_and_ci_log_not_gam$term <- rownames(main_analysis_sd_and_ci_log_not_gam)


main_analysis_sd_and_ci_log_not_gam$ci_95 <- 
  paste("95% CI = (", format(round(main_analysis_sd_and_ci_log_not_gam$conf.low, 3), nsmall = 3), ", ", 
        format(round(main_analysis_sd_and_ci_log_not_gam$conf.high, 3), nsmall = 3), ")", sep = "")

dwplot(main_analysis_sd_and_ci_log_not_gam[51:59,], colour = "black") +  
  ...
  scale_color_grey()    + 
  geom_text(main_analysis_sd_and_ci_log_not_gam[51:59,], 
...
```{r}
main_analysis_coef_of_interest_RR_not_gam <- main_analysis_sd_and_ci_log_not_gam[51:59,]
main_analysis_coef_of_interest_RR_not_gam$est_RR <- round(exp(main_analysis_coef_of_interest_RR_not_gam$estimate), 3)
main_analysis_coef_of_interest_RR_not_gam$est_RR_lb <- round(exp(main_analysis_coef_of_interest_RR_not_gam$conf.low), 3)
main_analysis_coef_of_interest_RR_not_gam$est_RR_ub <- round(exp(main_analysis_coef_of_interest_RR_not_gam$conf.high), 3)

main_analysis_coef_of_interest_RR_not_gam[,c("est_RR", "est_RR_lb", "est_RR_ub")]
...
sensitivity_anlys_lin_post_tx_sd_and_ci_logistic_smoothed_time_w_cov <- 
  compute_sd_and_CI_logistic(as.matrix(full_df_w_basis_functions_sensitivity_anlys_lin_post_tx_logistic_smoothed_time), 
...
sensitivity_anlys_post_tx_sd_and_ci_logistic_smoothed_time_subset_w_cov <- 
  compute_sd_and_CI_logistic(as.matrix(full_df_w_basis_functions_sensitivity_anlys_post_tx_logistic_smoothed_time_subset), 
...
#fit an OLS with smoothed time effects
sensitivity_analysis_model_log_two_yr_not_gam<-lm(log(prop_dead)~ State +
                                                     # s(Time_Period_ID, bs = "cr", by = as.factor(Region)) +
                                                    Time_Period_ID:as.factor(Region) + 
                                                     Naloxone_Pharmacy_Yes_Redefined +
                                                     Naloxone_Pharmacy_No_Redefined +
...
summary(sensitivity_analysis_model_log_two_yr_not_gam)

#examine fitted values

par(mfrow = c(2,2))
plot(sensitivity_analysis_model_log_two_yr_not_gam)
par(mfrow = c(1,1))
```
...
#compute the full dataset including basis functions
full_df_w_basis_functions_log_two_yr_not_gam <- model.matrix(sensitivity_analysis_model_log_two_yr_not_gam)

#estimate the 95% CI and SD
coefficient_values_log_fixed_two_yr_not_gam <- coef(sensitivity_analysis_model_log_two_yr_not_gam)
#type = "response" to get the estimated probabilities
sens_analysis_sd_and_ci_log_two_yr_not_gam <- compute_sd_and_CI_linear_link(full_df_w_basis_functions_log_two_yr_not_gam,
                                                                log(sensitivity_anlys_redefine_int_data$prop_dead),
                                                                coefficient_values_log_fixed_two_yr_not_gam,
                                                                k = ncol(full_df_w_basis_functions_log_two_yr_not_gam))

colnames(sens_analysis_sd_and_ci_log_two_yr_not_gam) <- c("conf.low", "estimate", "conf.high", "sd")
sens_analysis_sd_and_ci_log_two_yr_not_gam$term <- rownames(sens_analysis_sd_and_ci_log_two_yr_not_gam)


sens_analysis_sd_and_ci_log_two_yr_not_gam$ci_95 <- 
  paste("95% CI = (", format(round(sens_analysis_sd_and_ci_log_two_yr_not_gam$conf.low, 3), nsmall = 3), ", ", 
        format(round(sens_analysis_sd_and_ci_log_two_yr_not_gam$conf.high, 3), nsmall = 3), ")", sep = "")

dwplot(sens_analysis_sd_and_ci_log_two_yr_not_gam[51:59,], colour = "black") +  
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_vline(aes(xintercept = 0), linetype = "dashed") +
  labs(y = "Term", x = "Coefficients and 95% Confidence Intervals", 
  ...
  scale_color_grey()     + 
  geom_text(sens_analysis_sd_and_ci_log_two_yr_not_gam[51:59,], 
            mapping = aes(label = format(round(estimate, 3), nsmall = 3), x = 0.55, y = 9:1), size = 3) + 
  geom_text(sens_analysis_sd_and_ci_log_two_yr_not_gam[51:59,], 
            mapping = aes(label = ci_95, x = 0.9, y = 9:1), size = 3) +
  xlim(-.3, 1.1)
...
```{r}
two_yr_analysis_coef_of_interest_RR_not_gam <- sens_analysis_sd_and_ci_log_two_yr_not_gam[51:59,]
two_yr_analysis_coef_of_interest_RR_not_gam$est_RR <- round(exp(two_yr_analysis_coef_of_interest_RR_not_gam$estimate), 4)
two_yr_analysis_coef_of_interest_RR_not_gam$est_RR_lb <- round(exp(two_yr_analysis_coef_of_interest_RR_not_gam$conf.low), 4)
two_yr_analysis_coef_of_interest_RR_not_gam$est_RR_ub <- round(exp(two_yr_analysis_coef_of_interest_RR_not_gam$conf.high), 4)

two_yr_analysis_coef_of_interest_RR_not_gam[,c("est_RR", "est_RR_lb", "est_RR_ub")]
...
#type = "response" to get the estimated probabilities
sens_analysis_sd_and_ci_log_fixed_lin_time <- compute_sd_and_CI_linear_link(full_df_w_basis_functions_log_fixed_lin_time_2_yr,
                                                                log(sensitivity_anlys_redefine_int_data$prop_dead),
...
dwplot(sens_analysis_sd_and_ci_log_fixed_lin_time[51:59,], colour = "black") +  
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_vline(aes(xintercept = 0), linetype = "dashed") +
  labs(y = "Term", x = "Coefficients and 95% Confidence Intervals", 
       ...
       2 Year Exposure Intervention") + 
  scale_color_grey()     + 
  geom_text(sens_analysis_sd_and_ci_log_fixed_lin_time[51:59,], 
            mapping = aes(label = format(round(estimate, 3), nsmall = 3), x = 0.55, y = 9:1), size = 3) + 
  geom_text(sens_analysis_sd_and_ci_log_fixed_lin_time[51:59,], 
            mapping = aes(label = ci_95, x = 0.9, y = 9:1), size = 3) +
  xlim(-.3, 1.1)
...
```{r}
two_yr_analysis_coef_of_interest_RR <- sens_analysis_sd_and_ci_log_fixed_lin_time[51:59,]
two_yr_analysis_coef_of_interest_RR$est_RR <- round(exp(two_yr_analysis_coef_of_interest_RR$estimate), 3)
two_yr_analysis_coef_of_interest_RR$est_RR_lb <- round(exp(two_yr_analysis_coef_of_interest_RR$conf.low), 3)
two_yr_analysis_coef_of_interest_RR$est_RR_ub <- round(exp(two_yr_analysis_coef_of_interest_RR$conf.high), 3)

two_yr_analysis_coef_of_interest_RR[,c("est_RR", "est_RR_lb", "est_RR_ub")]
...
sens_analysis_sd_and_ci_log_fixed_lin_time_lin_post_tx_w_cov <-
  compute_sd_and_CI_linear_link(full_df_w_basis_functions_log_fixed_lin_time_lin_post_tx_2_yr,
...
table_of_RR_lin_eff_log_2_yr$estimate <- round(exp(table_of_RR_lin_eff_log_2_yr$estimate), 4)
...
table_of_RR_lin_eff_log_2_yr$ci_95 <- paste("95% CI = (", 
                                            format(round(table_of_RR_lin_eff_log_2_yr$conf.low, 4), nsmall = 4), ", ", 
                                            format(round(table_of_RR_lin_eff_log_2_yr$conf.high, 4), nsmall = 4), ")", sep = "")

# write.csv(table_of_RR_lin_eff_log_2_yr, "./Data/table_of_RR_lin_eff_log_2_yr_5_13_22.csv")
table_of_RR_lin_eff_log_2_yr
```
...
attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary <- attr_deaths_est_log_lin_time_lin_post_tx_2yr %>%
  group_by(year = year(Time_Period_Start)) %>%
  summarise(attr_deaths = sum(attr_deaths),
            attr_deaths_lb = sum(attr_deaths_lb),
            attr_deaths_ub = sum(attr_deaths_ub))

sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_lb)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_ub)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths)/
  sum(sensitivity_anlys_event_study_data_lin_post_tx_2yr$imputed_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_lb)/
  sum(sensitivity_anlys_event_study_data_lin_post_tx_2yr$imputed_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_ub)/
  sum(sensitivity_anlys_event_study_data_lin_post_tx_2yr$imputed_deaths)
...
ggplot(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary, aes(x = year)) + 
  # geom_point(aes(y = attr_deaths)) + 
  geom_line(aes(y = attr_deaths, linetype = "Estimate")) + 
  # geom_point(aes(y = attr_deaths_lb)) + 
  geom_line(aes(y = attr_deaths_lb, linetype = "95% CI")) + 
...
sens_analysis_sd_and_ci_log_fixed_lin_time_lin_post_tx_subset_w_cov <-
  compute_sd_and_CI_linear_link(full_df_w_basis_functions_log_fixed_lin_time_lin_post_tx_subset_2yr,
...
# write.csv(table_of_RR_lin_eff_log_2_yr_subset, "./Data/table_of_RR_lin_eff_log_2_yr_subset_5_13_22.csv")
table_of_RR_lin_eff_log_2_yr_subset
```
...
attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary <- attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr %>%
  group_by(year = year(Time_Period_Start)) %>%
  summarise(attr_deaths = sum(attr_deaths),
            attr_deaths_lb = sum(attr_deaths_lb),
            attr_deaths_ub = sum(attr_deaths_ub))

sum(attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths_lb)
sum(attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths_ub)
sum(attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths)/
  sum(sens_data_subset$imputed_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths_lb)/
  sum(sens_data_subset$imputed_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths_ub)/
  sum(sens_data_subset$imputed_deaths)

ggplot(attr_deaths_est_log_lin_time_lin_post_tx_subset_2yr_summary, aes(x = year)) + 
  # geom_point(aes(y = attr_deaths)) + 
  geom_line(aes(y = attr_deaths, linetype = "Estimate")) + 
  # geom_point(aes(y = attr_deaths_lb)) + 
  geom_line(aes(y = attr_deaths_lb, linetype = "95% CI")) + 
...
sens_analysis_sd_and_ci_logistic_fixed_lin_time_lin_post_tx_w_cov <-
  compute_sd_and_CI_logistic(full_df_w_basis_functions_logistic_fixed_lin_time_lin_post_tx_2_yr,
                    (sensitivity_anlys_event_study_data_lin_post_tx_2yr$prop_dead),
...
colnames(sens_analysis_sd_and_ci_logistic_fixed_lin_time_lin_post_tx) <- c("conf.low", "estimate", "conf.high", "exp_lb",
                                                                           "exp_coef", "exp_ub", "sd")
...
# write.csv(table_of_RR_lin_eff_logistic_2_yr, "./Data/table_of_RR_lin_eff_logistic_2_yr_5_13_22.csv")
table_of_RR_lin_eff_logistic_2_yr
```
...
attr_deaths_est_logistic_lin_time_lin_post_tx_2yr_summary <- attr_deaths_est_logistic_lin_time_lin_post_tx_2yr %>%
  group_by(year = year(Time_Period_Start)) %>%
  summarise(attr_deaths = sum(attr_deaths),
            attr_deaths_lb = sum(attr_deaths_lb),
            attr_deaths_ub = sum(attr_deaths_ub))

sum(attr_deaths_est_logistic_lin_time_lin_post_tx_2yr_summary$attr_deaths)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_2yr_summary$attr_deaths_lb)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_2yr_summary$attr_deaths_ub)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_2yr_summary$attr_deaths)/
  sum(sensitivity_anlys_event_study_data_lin_post_tx_2yr$imputed_deaths)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_2yr_summary$attr_deaths_lb)/
  sum(sensitivity_anlys_event_study_data_lin_post_tx_2yr$imputed_deaths)
...
ggplot(attr_deaths_est_logistic_lin_time_lin_post_tx_2yr_summary, aes(x = year)) + 
  # geom_point(aes(y = attr_deaths)) + 
  geom_line(aes(y = attr_deaths, linetype = "Estimate")) + 
  # geom_point(aes(y = attr_deaths_lb)) + 
  geom_line(aes(y = attr_deaths_lb, linetype = "95% CI")) + 
...
sens_analysis_sd_and_ci_logistic_fixed_lin_time_lin_post_tx_subset_w_cov <-
  compute_sd_and_CI_logistic(full_df_w_basis_functions_logistic_fixed_lin_time_lin_post_tx_subset_2yr,
                    (sens_data_subset$prop_dead),
...
colnames(sens_analysis_sd_and_ci_logistic_fixed_lin_time_lin_post_tx_subset) <- c("conf.low", "estimate", "conf.high", "exp_lb",
                                                                                  "exp_coef", "exp_ub", "sd")
...
table_of_RR_lin_eff_logistic_2_yr_subset$ci_95 <- paste("95% CI = (", 
                                                        format(round(table_of_RR_lin_eff_logistic_2_yr_subset$conf.low, 4), nsmall = 4), 
                                                        ", ", 
                                                        format(round(table_of_RR_lin_eff_logistic_2_yr_subset$conf.high, 4), nsmall = 4),
                                                        ")", sep = "")

# write.csv(table_of_RR_lin_eff_logistic_2_yr_subset, "./Data/table_of_RR_lin_eff_logistic_2_yr_subset_5_13_22.csv")
table_of_RR_lin_eff_logistic_2_yr_subset
```
...
attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr <- merge(attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr,
                                                                  date_data_subset_2yr, 
                                                                  by.x = "Time_Period", by.y = "Time_Period_ID")

attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr_summary <- attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr %>%
  group_by(year = year(Time_Period_Start)) %>%
  summarise(attr_deaths = sum(attr_deaths),
            attr_deaths_ub = sum(attr_deaths_ub),
            attr_deaths_lb = sum(attr_deaths_lb))

sum(attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths_lb)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths_ub)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths)/
  sum(sens_data_subset$imputed_deaths)
sum(attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr_summary$attr_deaths_lb)/
  sum(sens_data_subset$imputed_deaths)
...
ggplot(attr_deaths_est_logistic_lin_time_lin_post_tx_subset_2yr_summary, aes(x = year)) + 
  # geom_point(aes(y = attr_deaths)) + 
  geom_line(aes(y = attr_deaths, linetype = "Estimate")) + 
  # geom_point(aes(y = attr_deaths_lb)) + 
  geom_line(aes(y = attr_deaths_lb, linetype = "95% CI")) + 
...
pdf("./Figures/all_yr_effects_all_models_5_13_22.pdf")
...
pdf("./Figures/lives_saved_all_yr_5_13_22.pdf")
...
pdf("./Figures/two_yr_effects_all_models_5_13_22.pdf")
...
pdf("./Figures/lives_saved_all_2_yr_5_13_22.pdf")
...
pdf("./Figures/log_and_logistic_models_all_yr_effects_5_13_22_RR.pdf")
...
pdf("./Figures/lives_saved_main_models_5_13_22.pdf")
...
```{r}
cov_mat_main_analysis <- sensitivity_anlys_lin_post_tx_sd_and_ci_log_smoothed_time_w_cov[[2]]
cov_mat_two_yr <- sens_analysis_sd_and_ci_log_fixed_lin_time_lin_post_tx_w_cov[[2]]

plot_effects_only_log_all_yr_data <- data.frame(time_after_tx = rep(0:39, 2),
                                                coef = c(
                                                  sensitivity_anlys_lin_post_tx_sd_and_ci_log_smoothed_time$estimate[
                                                    sensitivity_anlys_lin_post_tx_sd_and_ci_log_smoothed_time$term ==
                                                      "Intervention_Redefined"] + 
                                                    sensitivity_anlys_lin_post_tx_sd_and_ci_log_smoothed_time$estimate[
                                                      sensitivity_anlys_lin_post_tx_sd_and_ci_log_smoothed_time$term ==
                                                        "lag_num_pd_w_tx"]*c(0:39),
                                                  sens_analysis_sd_and_ci_log_fixed_lin_time_lin_post_tx$estimate[
                                                    sens_analysis_sd_and_ci_log_fixed_lin_time_lin_post_tx$term == "int_2_yr_effect"] + 
                                                    sens_analysis_sd_and_ci_log_fixed_lin_time_lin_post_tx$estimate[
                                                      sens_analysis_sd_and_ci_log_fixed_lin_time_lin_post_tx$term ==
                                                        "lag_num_pd_w_tx"]*c(0:39)))

plot_effects_only_log_all_yr_data$conf.low <- c(plot_effects_only_log_all_yr_data$coef[1:40] - 
  1.96*sqrt(cov_mat_main_analysis["Intervention_Redefined", "Intervention_Redefined"] + 
      2*c(0:39)*cov_mat_main_analysis["Intervention_Redefined", "lag_num_pd_w_tx"] + 
        c(0:39)^2*cov_mat_main_analysis["lag_num_pd_w_tx", "lag_num_pd_w_tx"]),
  plot_effects_only_log_all_yr_data$coef[41:80] - 
  1.96*sqrt(cov_mat_two_yr["int_2_yr_effect", "int_2_yr_effect"] + 
      2*c(0:39)*cov_mat_two_yr["int_2_yr_effect", "lag_num_pd_w_tx"] + 
        c(0:39)^2*cov_mat_two_yr["lag_num_pd_w_tx", "lag_num_pd_w_tx"]))

plot_effects_only_log_all_yr_data$conf.high <- c(plot_effects_only_log_all_yr_data$coef[1:40] + 
  1.96*sqrt(cov_mat_main_analysis["Intervention_Redefined", "Intervention_Redefined"] + 
      2*c(0:39)*cov_mat_main_analysis["Intervention_Redefined", "lag_num_pd_w_tx"] + 
        c(0:39)^2*cov_mat_main_analysis["lag_num_pd_w_tx", "lag_num_pd_w_tx"]),
  plot_effects_only_log_all_yr_data$coef[41:80] +
...
plot_effects_only_log_all_yr_data$model  <- c(rep("Lasting Effect", 40), rep("Two-Year Effect", 40))



pdf("./Figures/only_log_models_all_yr_effects_5_17_22_RR.pdf")
ggplot(plot_effects_only_log_all_yr_data, mapping = aes(x = time_after_tx)) + 
  geom_line(mapping = aes(y = exp(coef), linetype = "Estimated Effect", color = model)) +
  geom_point(mapping = aes(y = exp(coef), linetype = "Estimated Effect", shape = model, color = model)) +
  geom_line(plot_effects_only_log_all_yr_data[plot_effects_only_log_all_yr_data$model == "Lasting Effect",],
            mapping = aes(y = exp(conf.low), linetype = "95% CI for Model with Lasting Effect", color = model)) + 
  geom_line(plot_effects_only_log_all_yr_data[plot_effects_only_log_all_yr_data$model == "Lasting Effect",],
            mapping = aes(y = exp(conf.high), linetype = "95% CI for Model with Lasting Effect", color = model)) + 
  geom_line(plot_effects_only_log_all_yr_data[plot_effects_only_log_all_yr_data$model == "Two-Year Effect",],
            mapping = aes(y = exp(conf.low), linetype = "95% CI for Model with Two-Year Effect", color = model)) + 
  geom_line(plot_effects_only_log_all_yr_data[plot_effects_only_log_all_yr_data$model == "Two-Year Effect",],
            mapping = aes(y = exp(conf.high), linetype = "95% CI for Model with Two-Year Effect", color = model)) + 
...
pdf("./Figures/only_log_models_all_yr_effects_lasting_effect_6_3_22_RR.pdf", width = 5, height = 5)
ggplot(plot_effects_only_log_all_yr_data[plot_effects_only_log_all_yr_data$model == "Lasting Effect",], 
       mapping = aes(x = time_after_tx)) + 
  geom_line(mapping = aes(y = exp(coef), linetype = "Estimated Effect")) +
  # geom_point(mapping = aes(y = exp(coef), linetype = "Estimated Effect")) +
  geom_line(plot_effects_only_log_all_yr_data[plot_effects_only_log_all_yr_data$model == "Lasting Effect",],
            mapping = aes(y = exp(conf.low), linetype = "95% CI")) + 
  geom_line(plot_effects_only_log_all_yr_data[plot_effects_only_log_all_yr_data$model == "Lasting Effect",],
            mapping = aes(y = exp(conf.high), linetype = "95% CI")) + 
  scale_linetype_manual(values = c("dashed", "solid")) + 
  labs(linetype = "",
       x = "Number of Periods After Treatment",
       y = "Estimated Risk Ratio",
...
colnames(attr_deaths_est_log_smoothed_time_lin_post_summary) <- 
  c("year", "attr_deaths", "attr_deaths_lb", "attr_deaths_ub")

attr_deaths_all_yr_effect_only_log <- rbind(cbind(attr_deaths_est_log_smoothed_time_lin_post_summary, 
                                                  "model" = "Lasting Effect"),
                                            cbind(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary, "model" = "Two-Year Effect"))


pdf("./Figures/lives_saved_only_log_models_5_17_22.pdf")
ggplot(attr_deaths_all_yr_effect_only_log, mapping = aes(x = year, shape = model, color = model)) + 
  # geom_point(aes(y = attr_deaths)) + 
  geom_line(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Lasting Effect",],
            mapping = aes(y = attr_deaths, linetype = "Estimated Effect")) + 
  geom_line(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Two-Year Effect",],
            mapping = aes(y = attr_deaths, linetype = "Estimated Effect")) + 
  geom_point(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Lasting Effect",],
             mapping = aes(y = attr_deaths)) +
  geom_point(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Two-Year Effect",],
             mapping = aes(y = attr_deaths)) +
  # geom_point(aes(y = attr_deaths_lb)) + 
  geom_line(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Lasting Effect",],
            mapping = aes(y = attr_deaths_lb, linetype = "95% CI for Model with Lasting Effect")) + 
  geom_line(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Lasting Effect",],
            mapping = aes(y = attr_deaths_ub, linetype = "95% CI for Model with Lasting Effect")) + 
  # geom_point(aes(y = attr_deaths_ub)) + 
  geom_line(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Two-Year Effect",],
            mapping = aes(y = attr_deaths_ub, linetype = "95% CI for Model with Two-Year Effect")) + 
  geom_line(attr_deaths_all_yr_effect_only_log[attr_deaths_all_yr_effect_only_log$model == "Two-Year Effect",],
            mapping = aes(y = attr_deaths_lb, linetype = "95% CI for Model with Two-Year Effect")) +
  labs(x = "Date", y = "Yearly Number of Lives Saved",
       # title = "Estimated Number of Lives Saved Using Smoothed Time Effects, 
       # Log Probability of Drug Overdose Death, 2 Year Linear Policy Effects",
       linetype = "",
...
sum(attr_deaths_est_log_smoothed_time_lin_post_summary$attr_deaths)/sum(main_analysis_data$imputed_deaths)
sum(attr_deaths_est_log_smoothed_time_lin_post_summary$attr_deaths_lb)/sum(main_analysis_data$imputed_deaths)
sum(attr_deaths_est_log_smoothed_time_lin_post_summary$attr_deaths_ub)/sum(main_analysis_data$imputed_deaths)

sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_lb)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_ub)

sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths)/sum(main_analysis_data$imputed_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_lb)/sum(main_analysis_data$imputed_deaths)
sum(attr_deaths_est_log_lin_time_lin_post_tx_2yr_summary$attr_deaths_ub)/sum(main_analysis_data$imputed_deaths)

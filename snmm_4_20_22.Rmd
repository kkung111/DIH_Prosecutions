...
When we included the past number of drug overdose deaths into the treatment initiation model, the $\psi$ that produced an $\alpha(\psi)$, where $\alpha(\psi)$ is the coefficient of $H_k(t)$ in the treatment initiation model, that is closest to 0 is -0.22. The 95% Confidence Interval is (-0.40, -0.08).
...
We first check the unbiased estimating equation where $H_{s,k}(t) \times \text{sum policies}$ is added to the treatment initiation model.
